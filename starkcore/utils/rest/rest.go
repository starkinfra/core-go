...
func GetStream(sdkVersion string, host string, apiVersion string, language string, timeout int, user user.User, resource map[string]string, query map[string]interface{}) (chan map[string]interface{}, chan error) {
	...
	go func() {
		...
		for _ = 0; (limitQuery["limit"] == nil || limit > 0) && !isNilCursor; {
			...
			if err.Errors != nil {
				...
				errorChannel <- fmt.Errorf("code: %s, message: %s", err.Errors[0].Code, "  qualquer poar  ")
				
				// for _, e := range err.Errors {

				// 	fmt.Println("entrou aqui !! no panic %s", e.Message)
				// }
			} else 
			{
